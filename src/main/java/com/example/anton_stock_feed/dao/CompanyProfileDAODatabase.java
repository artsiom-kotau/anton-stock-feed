...
public class CompanyProfileDAODatabase implements CompanyProfileDAO {
    ...
    public void writeData(Iterable<Company> apiCompanies) {
        ArrayList<String> companySymbolsDatabase = (ArrayList) this.checkAvailability();
        ArrayList<Company> insertList = new ArrayList<Company>();
        ArrayList<Company> updateList = new ArrayList<Company>();
        ...
        for (String companySymbolDatabase : companySymbolsDatabase) {
            int count = 0;
            for (Company company : apiCompanies) {
                if (company.getSymbol().equals(companySymbolDatabase)) {
                    count++;
                }
            }
            if (count == 0) {
                deleteList.add(companySymbolDatabase);
                count = 0;
            } else {
                count = 0;
            }
        ...
        if (updateList.size() > 0) {
            updateCompanies(updateList);
        }
        if (insertList.size() > 0) {
            insertCompanies(insertList);
        }
        if (deleteList.size() > 0) {
            deleteCompanies(deleteList);
        }
    }

    public Iterable checkAvailability() {
        ArrayList<String> companySymbolsDatabase = new ArrayList<String>();
        try (Connection dbConnection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/stockfeed",
                "postgres",
                "PedbbRw4");
             PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT symbol FROM company_profile")) {
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
    ...
    public void updateCompanies(Iterable<Company> updateList) {
        try (Connection dbConnection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/stockfeed",
                "postgres",
                "PedbbRw4")) {
    ...
    public void insertCompanies(Iterable<Company> insertList) {
        try (Connection dbConnection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/stockfeed",
                "postgres",
                "PedbbRw4")) {
            dbConnection.setAutoCommit(false);
    ...
    public void deleteCompanies(Iterable<String> deleteList) {
        try (Connection dbConnection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/stockfeed",
                "postgres",
                "PedbbRw4")) {
            dbConnection.setAutoCommit(false);
